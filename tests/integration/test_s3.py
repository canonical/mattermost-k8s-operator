...
"""S3 integration tests for Mattermost charm."""
...
from .conftest import MATTERMOST_PORT, generate_s3_config

logger = logging.getLogger(__name__)


def _get_mattermost_address(app: str, juju: jubilant.Juju) -> str:
    """Get a fresh Mattermost address from Juju status."""
    status = juju.status()
...
def test_s3_storage(
    ...
    # Wait for Juju status active AND Mattermost HTTP to be serving.
    # After integration, Mattermost restarts (pebble replan) and needs
    # time to come back up — Juju reports active before HTTP is ready.
    def all_active_and_mattermost_serving(status):
        if not jubilant.all_active(status):
            return False
        try:
            addr = _get_mattermost_address(app, juju)
            return requests.get(addr, timeout=5).status_code == 200
        except (requests.ConnectionError, requests.Timeout):
            return False

    juju.wait(all_active_and_mattermost_serving, timeout=1200)
